...
skmer=config["ErrorCorrection"]["seecerkmer"] if "seecerkmer" in config["ErrorCorrection"].keys() else kmer
base=config["Normalization"]["ornabase"]
okmer = config["Normalization"]["ornakmer"] if "ornakmer" in config["Normalization"].keys() else kmer
...
if norm:
	rule read_normalization:
		...
		shell:
			...
			if (((($size)) == 1))
			then
				ORNA -input {input[0]} -kmer {params.kmr} -base {params.bs} -type fa -output {outdir}/Normalized &> {outdir}/LogFiles/LogNormalization.txt
				mv {outdir}/Normalized.fa {output}
			else
				ORNA -pair1 {input[0]} -pair2 {input[1]} -kmer {params.kmr} -base {params.bs} -output {outdir}/Normalized/Normalized &> {outdir}/LogFiles/LogNormalization.txt			
			fi
			#rm -r *.h5
			"""
