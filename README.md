...
```
	>k_transcript_id
```
where k is the kmer sizewhich was used to generate the sequence.
Additional, the assembly folder would also contain intermediate sub-folders created by Kreation namely:
	(a).	Intermediate: Contains intermediate assembly generated after each kmer iteration
	(b).	Cluster: Contains sequence clusters generated after every kmer iteration
	(c).	Final: Contains a log file reporting the d_score after every kmer iteration.
